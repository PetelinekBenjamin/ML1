...
# Priprava učnih podatkov
def pripravi_podatke_za_ucenje(vrednosti, okno_velikost, stevilo_intervalov):
    X, y = [], []
    for i in range(len(vrednosti) - okno_velikost - stevilo_intervalov + 1):
        X.append(vrednosti[i:i+okno_velikost, :])
        y.append(vrednosti[i+okno_velikost:i+okno_velikost+stevilo_intervalov, :])  # Spremenjeno, da vsebuje vrednosti vseh značilnic
...
# velikost okna
okno_velikost = 5
# število intervalov, ki jih želite napovedati
stevilo_intervalov = 7

# Priprava učnih podatkov
X_train, y_train = pripravi_podatke_za_ucenje(train_data, okno_velikost, stevilo_intervalov)

# Priprava testnih podatkov
X_test, y_test = pripravi_podatke_za_ucenje(test_data, okno_velikost, stevilo_intervalov)
...
# Spremenjeno, da ima več izhodov, vsak za eno značilnico
output1 = Dense(1)(dropout)  # available_bike_stands
output2 = Dense(1)(dropout)  # precipitation_probability
output3 = Dense(1)(dropout)  # dew_point_2m
output4 = Dense(1)(dropout)  # apparent_temperature
output5 = Dense(1)(dropout)  # relative_humidity_2m
output6 = Dense(1)(dropout)  # temperature_2m

# Definicija modela
model_lstm = Model(inputs=inputs, outputs=[output1, output2, output3, output4, output5, output6])

# Kompilacija modela
model_lstm.compile(optimizer='adam', loss='mean_squared_error', metrics=['mae', 'mse'])

# učenje modela
history = model_lstm.fit(X_train, [y_train[:, i] for i in range(y_train.shape[1])], epochs=10, batch_size=32, validation_split=0.1, callbacks=[early_stopping], verbose=1)
